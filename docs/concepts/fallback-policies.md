...
|---|---|---|
| `retry` | Call the LLM again | Tasks like summarization where there's no good deterministic alternative |
| `deterministic` | Skip the LLM, return the preprocessed output | Tasks where the LLM is flaky. Falls back to whatever the tool chain produced |
